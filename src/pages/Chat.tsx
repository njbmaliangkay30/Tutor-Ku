              ...
              {messages.map((m, idx) => {
                ...
                return (
                  <div key={m.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-pgIn`}>
                    <div 
                      className={`max-w-[75%] md:max-w-[65%] rounded-2xl px-4 py-2.5 text-[13px] ${isMe ? 'bg-lime text-black rounded-tr-sm' : 'bg-bg-2 border border-border text-text-main rounded-tl-sm'}`}
                    >
                      {m.content}
                    </div>
